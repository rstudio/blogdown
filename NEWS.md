...
- When a website is built or served for the first time in an R session, **blogdown** will detect `.html` files that seem to be generated by clicking the Knit button in RStudio with **blogdown** < v0.21. Such `.html` files should be deleted, since the Knit button only works with **blogdown** >= v0.21. A message will be shown if such `.html` files are detected. You may suppress the detection via the global option `options(blogdown.check.html = FALSE)` if you think it signals a false alarm.
