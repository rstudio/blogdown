...
GitLab's CI/CD service uses the instructions stored in the file `.gitlab-ci.yml` in the repository. Here is a sample configuration file `.gitlab-ci.yml` from the repository https://rgaiacs.gitlab.io/blogdown-gitlab:
...
before_script:
  - apt-get update
  - apt-get -y install pandoc r-base-core
