...
[GitLab](http://gitlab.com/) is a very popular way to host the source code of your project.
GitLab has a [built in Continuous Integration & Deployment service](https://about.gitlab.com/features/gitlab-ci-cd/)
that can be used to host static websites, named [GitLab Pages](https://about.gitlab.com/features/pages/).
The major advantage of using GitLab Pages is that
GitLab Continuous Integration & Deployment service will compile all your Rmd posts
instead of your local computer
**and any generated content, such as HTML files, will be stored in the Git repository**.

Before I show you how, I’d like to mention two issues that you should be aware of:

- It is somewhat unpredictable the output
  because it is fully automatic
  and you do not have a chance to see the new content or results to be published.
  There are many factors that could affect building the site:
  the R version,
  availability of certain R packages,
  system dependencies, and network connection, etc.
- The time required to compile all Rmd files may be very long
  and cause timeouts on GitLab,
  depending on how time-consuming your R code is.

The first problem can be solved by specifying the version of the package you are using
on the installation instructions.
The second problem can be solved by using caching mechanism.

Like GitHub Pages,
you can have project websites in separate repositories,
e.g., you may have two independent websites `https://username.gitlab.io/proj-a/` and `https://username.gitlab.io/proj-b/`,
corresponding to GitLab repositories `https://gitlab.com/username/proj-a/` and `https://gitlab.com/username/proj-b/`, respectively.
Is also possible to associate each repository with a domain or subdomain name,
for example,
you may replace use `https://proj-a.username.com/` and `https://proj-b.username.com/`.

GitLab Continuous Integration & Deployment service uses the instructions stored on the file `.gitlab-ci.yml` at the root of the repository.
Here is the configuration file, `.gitlab-ci.yml`, for https://rgaiacs.gitlab.io/blogdown-example:
...
before_script:
  - apt-get update
  - apt-get -y install curl libcurl4-openssl-dev openssl libssl-dev git pandoc r-base-core r-recommended
...
The key is that we install Hugo via `blogdown::install_hugo()` and build the site via `blogdown::build_site()`.
There are a few more things to explain and emphasize in `.gitlab-ci.yml`:

- The `image` option specifies what [Docker](https://www.docker.com/) image will be use as a start point. We are using a Debian image but any image from [Docker Hub](https://hub.docker.com/) can be used.
- The `before_script` option specifies how to install blogdown on the image specify before. If you change `image` to a non-Debian image you need to replace the `apt-get` lines with similar ones for the package manager provided by the image you specify.
- The `only` option specifies that only changes in the master branch will trigger building on Travis.

[GitLab Continuous Integration documentation](https://docs.gitlab.com/ee/ci/README.html) provides more information about `.gitlab-ci.yml`.
[GitLab Pages documentation](https://docs.gitlab.com/ee/user/project/pages/) provides information for set up a custom domains and other things.
[GitLab Pages blogdown-example](https://gitlab.com/rgaiacs/blogdown-example) provides a reference set up.