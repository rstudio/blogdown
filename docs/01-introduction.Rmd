...
To follow along with this book, we recommend using **blogdown** `r packageVersion("blogdown")[, 1:2]` or higher. 
...
```{r include=FALSE}
deps <- desc::desc(package = "blogdown")$get_deps()
rmd_version <- strsplit(deps[deps$package == "rmarkdown", 3], " ")[[1]][2]
```


Do you have an old version of **rmarkdown**? The updated version of the **blogdown** package imports the **rmarkdown** package version `r rmd_version`. If your **rmarkdown** version is out of date, it will be updated using the RStudio IDE. However, if you are experiencing problems or are not using RStudio, compare your version against ours and upgrade if necessary.
...
| `themes/`     | Hugo theme assets like HTML layout files, CSS, etc. Do not edit!    |
