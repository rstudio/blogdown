...
To upgrade or reinstall Hugo, you may pass `force = TRUE` to `install_hugo()`. You can check the installed Hugo version via `blogdown::hugo_version()`, and find the latest version of Hugo at <https://github.com/gohugoio/hugo/releases>.
...
1. Click the RStudio addin "Serve Site" to preview the site in RStudio Viewer. This only needs to be done once every time you open the RStudio project, or restart your R session.
