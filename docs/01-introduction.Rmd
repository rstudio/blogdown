...
- "Insert Image": This addin allows you to easily insert any image in your computer to your currently opened post^[Check <http://lcolladotor.github.io/2018/03/07/blogdown-insert-image-addin> for a more in depth explanation and for how to insert an image without using this addin]. See Figure \@ref(fig:insert-image) to check how the addin looks. This addin copies the image to the final location for your post files and adds the markdown/HTML syntax for linking to the image. The addin lets you specify the width and height of the image (with any valid CSS syntax) as well as the alternative text. The addin shows the final figure location which you can edit if necessary. If the image already exists, it asks you whether you want to overwrite it as in Figure \@ref(fig:overwrite-image): when in doubt, change the name in the _target file path_ text input to avoid a conflict with previous images you have inserted.
...
```{r insert-image, fig.cap='Overwrite the image you are inserting in your blog post.', fig.align='center', out.width='70%', echo=FALSE}
