...
The blogdown R package uses global options, specified in a `.Rprofile` file, to help users customize how blogdown works. When you start a new site, blogdown adds a project-level `.Rprofile` file in the root directory of your website project.

Options should be set using `options(name = value)`, and those included in the new blogdown site `.Rprofile` file are presented in Table \@ref(tab:global-options-new).

```{r global-options-new, echo=FALSE}
knitr::kable(matrix(c(
  'blogdown.serve_site.startup', 'FALSE', 'Do not serve the site on RStudio startup',
  'blogdown.knit.on_save', 'TRUE', 'Knit Rmd files automatically on save; if FALSE, you will need to knit each Rmd file to render',
  'blogdown.method', '.html', 'Knit .Rmd to .html with Pandoc; set to "markdown" to bypass Pandoc, knit to .md, and use Hugo Goldmark to render',
  'blogdown.hugo.version', 'a valid Hugo version', 'Versions specified as a string like "0.80.0" detected using `blogdown::hugo_version()`',
  NULL
), ncol = 3, byrow = TRUE, dimnames = list(NULL, c('Option name', 'Default', 'Meaning'))), booktabs = TRUE, caption = 'Global options for new blogdown sites.')
```

We recommend that you set these options in your R startup profile file. If you've never used a startup profile file before, you can check out the help page `?Rprofile`. Here, we provide a brief but incomplete introduction to orient you quickly.
...
If you want to add a profile file to an existing blogdown project, or you created a new website without using `blogdown::new_site()`, you can create a boilerplate version using `blogdown::config_Rprofile` in your R console:

```{r eval=FALSE}
blogdown::config_Rprofile()
```

This is the easiest way to create or modify a per-project profile file.
At the top of the file in a new blogdown website project, you'll see this:
...
First, note the top message! The next section of code is necessary to execute both your project profile and global profile, if one exists. R only reads one startup profile file. For example, if you have a `.Rprofile` under the current directory and a global `~/.Rprofile`, only the former one will be executed when R starts up from the current directory. This code is provided for you so you may execute both a global and per-project profile file.

Below that, you'll set your options. These can be stacked on separate lines, or you may use commas to list multiple options together:
...
It is up to you how to format your profile file- neither way is wrong, and both should work. The blogdown options provided in the boilerplate profile file are just a subset of the options available. Depending on your personal preferences and the theme you choose, you may wish to set more global options as you work on your website. For example:

```{r global-options-2, echo=FALSE}
knitr::kable(matrix(c(
  'blogdown.author', '', 'The default author of new posts',
  'blogdown.ext', '.md', 'The default extension of new posts; options are .md, .Rmd, and .Rmarkdown',
  'blogdown.subdir', 'post', 'Which subdirectory under content/ should posts go? This depends on your theme',
