...
If you cannot use `install_hugo()` for any reason, you may download a copy of the Hugo executable, put it anywhere in your system, and set the global option `blogdown.hugo` to be the path to the executable, e.g., `options(blogdown.hugo = '~/Downloads/hugo_0.20.1/hugo')`. This may be useful for you to use a specific version of Hugo for a specific website,^[You can set this option per project. See Section \@ref(global-options) for details.] or store a copy of Hugo on a USB Flash drive along with your website.
...
knitr::kable(matrix(c(
  ...
  'blogdown.hugo', '', 'the path to the Hugo executable',
