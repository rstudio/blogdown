...
To give you an idea of what a processor function may look like, I provided two simple helper functions in **blogdown**:
...
The first function substitutes two or more empty lines with a single empty line. The second function replaces links of the form `[url](url)` with `<url>`. There is not anything wrong with excessive empty lines or the syntax `[url](url)`, though. These helper functions may make your Markdown text a little cleaner.

The YAML metadata of your posts may not be completely clean, especially when your Jekyll website was actually converted from an ealier WordPress website. There is an internal helper function `blogdown:::modify_yaml()`, which may help you clean up the metadata. For example, below is the YAML metadata of a blog post of Simply Statistics when it was built on Jekyll:
...
You can pass a file path to `modify_yaml()`, define new YAML values (or functions to return new values based on the old values), and decide which fields to preserve (`.keep_fields`). You may discard empty fields via `.keep_empty = FALSE`. The processed YAML medata is below, which looks much cleaner:
