...
We have created a minimal **blogdown** example website to demonstrate how to use R Markdown features (https://blogdown-demo.rbind.io). Some example posts are:

- [How to add cites in posts](https://blogdown-demo.rbind.io/2017/08/28/adding-citations-to-posts/)
- [How to add R code to posts](https://blogdown-demo.rbind.io/2017/10/06/adding-r-code-to-r-markdown-posts/)
- [How to include figures and other images in posts](https://blogdown-demo.rbind.io/2017/10/06/adding-figures-and-other-images-to-posts/)

### Hugo resources

Here we provide some useful links for beginning Hugo and **blogdown** users:

- [Hugo directory structure](https://gohugo.io/getting-started/directory-structure/) explains the basics of what goes where in a Hugo project
- [Creating and organizing content in Hugo](https://gohugo.io/content-management/organization/) explains the relationship between your site's content structure and the output file paths/URL structures that Hugo renders
- [Hugo configuration variables](https://gohugo.io/getting-started/configuration/) provides a list of all possible options for your `config.toml` file (scroll down to the *All Variables, TOML* section)
- [Emoji cheat sheet](https://www.webpagefx.com/tools/emoji-cheat-sheet/) (note: your `config.toml` file must include `enableEmoji = TRUE`)
- [Customizing Hugo themes](https://gohugo.io/themes/customizing/) explains a key concept for Hugo users, which is to *override* rather than *edit* theme files (static files, template files, and archetypes)

For advanced users, here are some additional resources:

- [Hugo lookup orders](https://gohugo.io/templates/lookup-order/) explains when and where Hugo looks for files in your site's directory structure
- [Hugo homepages](https://gohugo.io/templates/homepage/) explains how to change the homepage of a site (regardless of theme), and the lookup order for the homepage template
- [Hugo partial templates](https://gohugo.io/templates/partials/) explains lookup orders for partials and variable scoping
- [Hugo menu variables](https://gohugo.io/variables/menus/) provides a list of the built-in options for customizing variables displayed in a Hugo menu
...
Entire books have been written about SEO, not to mention the many companies that are in the business of offering (paid) technical and strategic advice to help get sites atop search-engine rankings. If you're interested in finding out more, a good place to start is the Google Search Engine Optimization Starter Guide (https://static.googleusercontent.com/media/www.google.com/en//webmasters/docs/search-engine-optimization-starter-guide.pdf).
