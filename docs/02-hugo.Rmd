...
"Static site generator" refers to the way that a website's content is generated. On dynamically generated sites, the content is rendered every time a user visits the site. Alternatively, on statically generated sites, the content is only rendered when the content changes, making it ready to access when a user visits the site^[More information here: https://gohugo.io/overview/introduction/]. 

Hugo is one example of a static site generator. It provides the framework for a variety of website types including, but certainly not limited to, blogs.

We're fans of Hugo because it's
...
```{r lithium, fig.cap='Files and Folders created when you create a new_site() using blogdown', echo=FALSE, fig.align='center', out.width='90%'}
knitr::include_graphics('images/folder-structure.png')
```

## Configuration

The first file that Hugo uses to generate a website is the configuration or config file. It's worth mentioning here that technically speaking, Hugo could generate your website without a config file if your files and folders are in the correct locations. Regardless, Hugo will look for a config file in the root directory of your website, so it's best to make sure that you have one. Generally speaking, this is the only file that Hugo expects to be in the root directory. 

As its name suggests, the config file is where you tell Hugo exactly how to render your webpage and display your menus. It's also the location of other global parameters like links to your social networks, your site's name, and the base URL for your website. 
...
This is a table in the TOML language and Hugo uses them to fill in information on other pages within your site. For instance, the above table will be populate the Site.Social variable in the templates (more information on this in [section 2.4](##Templates))
...
In TOML, double brackets are used to indicate an array of tables. Hugo interprets this information as a menu. If the code above was found in a config.toml file, the resulting website would have links to a blog, categories, and about page in the site's main menu. The location and styling of that menu is specified elsewhere, but the names of each menu's choices and the links to each section are defined here. 
 
The config.toml file varies between each theme. Make sure that when you choose a theme, you read its documentation thoroughly to get an understanding of what each of the configuration options does (more on themes in section [2.3](##Themes))
