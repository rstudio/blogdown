...
- `ignoreFiles`: A list of filename patterns (regular expressions) for Hugo to ignore\index{ignoreFiles} certain files when building the site. I recommend that you specify at least these patterns `["\\.Rmd$", "\\.Rmarkdown$", "_cache$"]`. You should ignore `.Rmd` files because Hugo should only recognize their output files (such as `.html` or `.md` files). Directories with the suffix `_cache` should be ignored because they contain auxiliary files after an `Rmd` file is compiled, which are useless to Hugo.
