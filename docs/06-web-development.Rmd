...
The Cascading Stylesheets (CSS) language is used to describe the look and formatting of documents written in HTML or XML. CSS is responsible for the visual style of your site. CSS is a lot of fun to play with, but it can also easily steal your time.

In the Hugo framework (https://gohugo.io/tutorials/creating-a-new-theme/), CSS is one of the major "non-content" files that shapes the look and feel of your site (the others are images, JavaScript, and Hugo templates). What does the ["look and feel"](https://en.wikipedia.org/w/index.php?title=Look_and_feel&oldid=731052704) of a site mean? "Look" generally refers to static style components including, but not limited to:
...
However, this method is generally not preferred for [numerous reasons](https://stackoverflow.com/questions/12013532/inline-style-tags-vs-inline-css-properties). 
...
We can use this class attribute as a selector in our CSS. Let's say we want our favorite teas to be bolded and have a background color of yellow, so our CSS would look like this:
...
Now, suppose you want every list item to be italicized with no background color, so you set up another rule:
...
Because each HTML element may match several different selectors, the CSS standard determines which set of rules has precedence for any given element, and which properties to inherit. This is where the cascade algorithm comes into play. For example, take this simple unordered list:

```html
<ul id="book-characters">
   <li>Bananas Gorilla</li>
   <li>Lowly the worm</li>
   <li>Captain Anteater</li>
   <li>Waveband the radioman</li>
</ul>
```

Now, let's say we want to highlight main characters here, so we'll use a class attribute.

```html
<ul id="book-characters">
   <li class="main">Bananas Gorilla</li>
   <li>Lowly the worm</li>
   <li class="main">Captain Anteater</li>
   <li>Waveband the radioman</li>
</ul>
```

We can use this class attribute as a selector in our CSS. Let's say we want the main characters to be bolded and have a background color of yellow, so our CSS would look like this:
...
Now, suppose you want all list items to be italicized with no background color, so you set up another rule:
...
If you play with this code (which you can do easily using sites like http://jsbin.com, https://jsfiddle.net, or https://codepen.io/pen/), you'll see that the two main characters names are still highlighted in yellow. This is because the CSS rule about `.main` as a class is more specific than the rule about `li` type elements. To override the `.main` rule, you need to be as specific as you can be when choosing your group of selectors:

```css
ul#book-characters li.main {
  background-color: white;
}
```
