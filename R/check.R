...
#'   files. It detects \file{.html} files that seem to be generated by clicking
#'   the Knit button in RStudio with \pkg{blogdown} < v0.21. Such \file{.html}
#'   files should be deleted, since the Knit button only works with
#'   \pkg{blogdown} >= v0.21.
#' @rdname check_site
#' @export
check_content = function() {
  ...
  msg1('Searching for posts with future publish dates')
  ...
  msg1('Searching for draft posts')
  ...
  msg1('Checking for R Markdown posts that have both .md/.markdown and .html output files')
  msg1('Checking for R Markdown posts that are older than their output files')
  check_garbage_html()
}
