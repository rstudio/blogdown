...
moon_reader = function(
  ...
  pre_knit <- function(input, ...) {
    ...
    if (is.null(yaml_header$type)) {
      stop("\nFile name suggest these are slides but type is not set to 'slides' in YAML.\nPlease fix this and try again.\n", call. = FALSE)
    } else if (yaml_header$type != "slides") {
      stop("\nFile name suggest these are slides but type is set as '", yaml_header$type, "' instead of 'slides'.\nPlease fix this and try again.\n", call. = FALSE)
    }
