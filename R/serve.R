...
preview_site = function(..., startup = FALSE) {
  ...
  if (startup) {
    ...
    if (is.null(init <- getOption('blogdown.initial_files.open'))) {
      if (init <- is.null(rstudioapi::getSourceEditorContext())) {
        message2(
          ...
          'options(blogdown.initial_files.open = FALSE) in your .Rprofile.'
        )
      }
    }
